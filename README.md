...
Пять хоумскрин-виджетов поверх vault [Obsidian](https://obsidian.md/) с плагином
...
- **Сейчас → Далее** — узкая лента-строка (4×1, сжимается до 3×1): что идёт **сейчас**
  («Сейчас: Демо клиенту · до 16:00 📍 Переговорка») и что **следующее** сегодня
  («→ 16:15 Встреча с командой»). Только абсолютное время (до `HH:mm` / `HH:mm`), без
  «осталось N минут» — на границах событий виджет обновляется точечно, точным будильником.
  Тап по строке открывает **шторку деталей**; при 3×1 место `📍` прячется.
- **Захват** — квадратная кнопка `＋` (1×1): тап открывает тот же **оверлей быстрого
  захвата** с поднятой клавиатурой. Пространство назначения выбирается при добавлении
  (по умолчанию **«Все»** → пишет в «Общее»); данные vault виджету не нужны.
...
2. Найдите **«GTD · Сегодня»**, **«GTD · Сейчас»**, **«GTD · Агенда»**,
   **«GTD · Входящие»** и/или **«GTD · Захват»** и перетащите на экран.
3. Размер виджета можно менять — все списки скроллируются; «Сейчас» ужимается с 4×1 до
   3×1 (тогда место `📍` прячется), «Захват» — фиксированная кнопка 1×1.
...
### Настройка пространства для «Захвата»

При добавлении виджета «Захват» откроется выбор **пространства назначения** (по образцу
«Входящих», первым — дефолт **«Все»**). Тап по кнопке `＋` пишет во входящие выбранного
пространства (при «Все»/«Общее» — в «Общее»). Выбор запоминается для каждого экземпляра.
Виджет «Сейчас» конфигурации не требует — он берёт общий кэш «Сегодня».
